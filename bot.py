...
def welcome(m):
    ...
    if str(banlist) == 'False':
       ...
       markup.add(nn)
       oo = types.InlineKeyboardButton("Channel", url='https://telegram.me/TeleNevisi')
       markup.add(oo)
       id = m.from_user.id
...
def callback_inline(call):
     ...
     if call.message:
        if call.data == "pouria":
            bot.answer_callback_query(callback_query_id=call.id, show_alert=True, text="CyberBot Created By @R00T3 And Written In Python")
     if call.message:
        if call.data == "short":
...
def query_text(query):
    ...
    hi = types.InlineQueryResultArticle('7', 'Music', types.InputTextMessageContent('*@cybermirbot music [Music name]*', parse_mode='Markdown'), thumb_url=hi_tmp)
